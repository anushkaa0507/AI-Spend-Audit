...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: leadData, error: leadError } = await supabase
      .from("leads")
      .insert([{ audit_id: auditId, email, company, role, team_size: teamSize }])
      .select()
      .single();
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:Inter,system-ui,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#09090f;color:#e9ddff;border-radius:16px">
          ...
          ${highSavings ? `
          <div style="background:rgba(74,222,128,0.1);border:1px solid rgba(74,222,128,0.3);border-radius:12px;padding:20px;margin-top:20px">
            <p style="margin:0;font-weight:700;color:#4ade80">High-savings case detected</p>
            <p style="margin:8px 0 0;color:rgba(203,196,210,0.7);font-size:14px">A Credex consultant will reach out to discuss how we can help capture these savings through vendor credits and enterprise pricing strategy.</p>
          </div>` : ""}
